...
class MEPP_Common_Polyhedron : public CGAL::Polyhedron_3<kernel,items>
{
	...
	public:
        ...
        void Normalise()
        {
            ...
            Vector center((xmax()+xmin())/2.,(ymax()+ymin())/2.,(zmax()+zmin())/2.);
            for(Vertex_iterator	pVertex	= this->vertices_begin();pVertex!= this->vertices_end();pVertex++)
            {
                pVertex->point()=pVertex->point()-center;//Point(pVertex->point().x()-xmin(),pVertex->point().y()-ymin(),pVertex->point().z()-zmin());
        ...
        void write_log(string output_name)
        {
            std::ofstream stream(output_name.c_str());
            for (Vertex_iterator pVertex = this->vertices_begin(); pVertex != this->vertices_end(); pVertex++)
            {
                stream << 'v' << ' ' << pVertex->point().x() << ' ' << pVertex->point().y() << ' ' << pVertex->point().z() << std::endl;

                stream << "tag " << pVertex->tag() << std::endl;

                // normal
                stream << "normal " << pVertex->normal().x() << ' '  << pVertex->normal().y() << ' '  << pVertex->normal().z() << std::endl;

                // color
                stream << "color " << pVertex->color(0) << ' ' << pVertex->color(1) << ' ' << pVertex->color(2) << std::endl;

                // texture coordinates
                stream << "tex " << pVertex->texture_coordinates(0) << ' ' << pVertex->texture_coordinates(1) << std::endl;

                #ifdef BUILD_component_Curvature
                stream << "KminCurv " << pVertex->KminCurv << " KmaxCurv " << pVertex->KmaxCurv << std::endl;
                stream << "VKminCurv " << pVertex->VKminCurv.x() << " " << pVertex->VKminCurv.y() << " " << pVertex->VKminCurv.z() << " " << std::endl;
                stream << "VKmaxCurv " << pVertex->VKmaxCurv.x() << " " << pVertex->VKmaxCurv.y() << " " << pVertex->VKmaxCurv.z() << " " << std::endl;
                #endif

                #ifdef BUILD_component_MSDM2
                stream << "MSDM_Local " << pVertex->MSDM2_Local << std::endl;
                stream << "match " << pVertex->match << std::endl;
                stream << "curvmatch " << pVertex->curvmatch << std::endl;
                #endif
                stream << std::endl;
            }
            stream.flush();
